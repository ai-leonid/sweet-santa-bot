...
1.  **Layout & Providers**
    -   [x] Обертка для инициализации Telegram WebApp.
    -   [x] React Query или просто `useEffect` для загрузки данных.

2.  **Главная страница (Home)**
    -   [x] Список активных игр пользователя.
    -   [x] Кнопка "Создать новую игру".
    -   [x] Обработка deep-link параметров (если перешли по ссылке-приглашению, сразу предлагать вступить).

3.  **Страница игры (Lobby)**
    -   [x] Заголовок игры.
    -   [x] Список участников (разделение на "В сети" и "Оффлайн").
    -   **Для создателя**:
        -   [x] Кнопка "Добавить оффлайн участника".
        -   [x] Кнопка настройки (шестеренка) напротив каждого участника -> Открывает модал Стоп-листа.
        -   [x] Кнопка "Начать жеребьевку" (если участников > 2).
    -   **Для обычного участника**:
        -   [x] Видит список, ждет начала.
    -   **После жеребьевки**:
        -   [x] Кнопка "Посмотреть результат" (для себя).
        -   [x] Если создатель: Кнопка "Результат оффлайн игроков".

4.  **Модалка Стоп-листа**
    -   [x] Выбор "Кому не может дарить {Имя}".
    -   [x] Чекбокс "Взаимоисключение".
    -   [x] Список текущих ограничений с кнопкой удалить.

5.  **Просмотр результатов**
    -   [x] **Личный**: Карточка с текстом "Ты даришь подарок..." и спойлер (компонент, скрывающий текст до клика).
    -   [x] **Оффлайн**: Список оффлайн игроков. Клик по игроку -> открывается его результат под спойлером.
